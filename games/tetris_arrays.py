...
def addToBoard(board, piece):
	...
	'''
	left = np.where(np.sum(PIECES[piece['shape']][piece['rotation']] != '', axis=0) > 0)[0][0] 
	right = np.where(np.sum(PIECES[piece['shape']][piece['rotation']] != '', axis=0) > 0)[0][-1] +1
	top = np.where(np.sum(PIECES[piece['shape']][piece['rotation']] != '', axis=1) > 0)[0][0] 
	bottom = np.where(np.sum(PIECES[piece['shape']][piece['rotation']] != '', axis=1) > 0)[0][-1] +1
	
	try:
		miny = np.max([0, piece['y']+top])
		top2 = -piece['y'] +miny
		p = np.place(PIECES[piece['shape']][piece['rotation']][left:right, top:bottom], PIECES[piece['shape']][piece['rotation']][left:right, top:bottom] != '' , piece['color'])
		board[:, piece['x']+left:piece['x']+right, miny:piece['y']+bottom] = np.core.defchararray.add(board[:, piece['x']+left:piece['x']+right, piece['y']+top:piece['y']+bottom], p)				
...
if __name__ == '__main__':
	main(screenwidth=640, screenheight=480, frames_per_second=32)
