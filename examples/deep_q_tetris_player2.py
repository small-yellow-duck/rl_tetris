...
class DeepQTetrisPlayer(PyGamePlayer):
	ACTIONS_COUNT = 6  # number of valid actions. In this case still, left, right, space, down, up
	FUTURE_REWARD_DISCOUNT = 0.99  # decay rate of past observations
	OBSERVATION_STEPS = 32*500 #50000.	# time steps to observe before training
	...
	MINI_BATCH_SIZE = 20 #200  # size of mini batches
	STATE_FRAMES = 32 #4  # number of frames to store in the state
	OBS_LAST_STATE_INDEX, OBS_ACTION_INDEX, OBS_REWARD_INDEX, OBS_CURRENT_STATE_INDEX, OBS_TERMINAL_INDEX = range(5)
	SAVE_EVERY_X_STEPS = 10000
	LEARN_RATE = 1e-6
	STORE_SCORES_LEN = 200.
	SCREEN_WIDTH = 40 #640/5#120 #
	SCREEN_HEIGHT = 40 #480/5#160 #

	def __init__(self,
				 # to see a trained network change checkpoint_path="deep_q_half_pong_networks_40x40_8" and
				 # playback_mode="True"
				 checkpoint_path="deep_q_tetris_networks",
				 playback_mode=True,
				 verbose_logging=True,
				 manual_training=False):
		"""
		Example of deep q network for tetris
		...
		self._manual_training = manual_training
		self.last_score = 0
		super(DeepQTetrisPlayer, self).__init__(force_game_fps=8, run_real_time=playback_mode)
		...
		print 'self._action', self._action.get_shape()
	...
	def get_keys_pressed(self, screen_array, reward, terminal):
		...
		#_, screen_binary = cv2.threshold(cv2.cvtColor(screen_array, cv2.COLOR_BGR2GRAY), 1, 255, cv2.THRESH_BINARY)
		screen_binary = cv2.cvtColor(screen_array, cv2.COLOR_BGR2GRAY)
		
		#resize game screen to something smaller to feed to the neural network
		screen_binary = cv2.resize(screen_binary, (self.SCREEN_WIDTH, self.SCREEN_HEIGHT))
		...
		if self._last_state is None:
			...
			return DeepQTetrisPlayer._key_presses_from_action(self._last_action)
		...
		if not self._playback_mode:
			...
			if reward > 0.0:
				...
				print(self._time, self._probability_of_random_action, reward, sum(self._last_scores) / self.STORE_SCORES_LEN)
	...
	def _choose_next_action(self):
		...
		if self._manual_training:
			action_index = 0 # set this to get keystroke from user
			...
			action_index = random.randrange(self.ACTIONS_COUNT-1)
		else:
	...
	def _create_network(self):
		...
		#feed_forward_weights_1 = tf.Variable(tf.truncated_normal([3*256, 256], stddev=0.01))
		...
		#hidden_convolutional_layer_3_flat = tf.reshape(hidden_max_pooling_layer_2, [-1, 3*256])
	...
	def get_feedback(self):
		from games.tetris import score
	...
	def get_feedback(self):
		...
		self.last_score = temp
		
		return temp, terminal
			

	def start(self):
		super(DeepQTetrisPlayer, self).start()

		#from games.tetris import run
		#run(screen_width=self.SCREEN_WIDTH, screen_height=self.SCREEN_HEIGHT)
		#games.tetris.run()
		#games.tetris.main(WINDOWWIDTH=self.SCREEN_WIDTH, WINDOWHEIGHT=self.SCREEN_HEIGHT)
		games.tetris.main(screenwidth=640, screenheight=480, frames_per_second=2)

if __name__ == '__main__':
	# to see a trained network add the args checkpoint_path="deep_q_half_pong_networks_40x40_8" and
	# playback_mode="True"
	player = DeepQTetrisPlayer(checkpoint_path='deep_q_tetris_networks', playback_mode=False, manual_training=False)
	#player = DeepQTetrisPlayer(checkpoint_path='deep_q_tetris_networks_40x40_8', playback_mode=True)
	player.start()
